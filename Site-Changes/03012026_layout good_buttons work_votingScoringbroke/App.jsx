      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 15px' }}>
        ...
        {isAdmin && (
          <div style={{ border: '2px solid #ef4444', padding: '15px', borderRadius: '20px', marginBottom: '30px', background: 'rgba(239, 68, 68, 0.1)' }}>
            <button 
              onClick={async () => { 
                if (!window.confirm("RESET ENTIRE GAME? This clears all points and resets to Ep 2.")) return;
                const { error } = await supabase.rpc('season_hard_reset'); 
                if (error) alert("Reset Error: " + error.message);
                else { setSelectedCandidate(null); setHasVoted(false); fetchData(); alert("Game Reset."); }
              }} 
