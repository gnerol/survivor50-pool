...
export default function App() {
      ...
      const prevHardLocked = useRef(false);
      ...
      // --- Rock Solid Slam Animation Logic ---
      useEffect(() => {
      ...
      const openTribeEditor = () => {
            ...
            const currentTribes = Object.values(uniqueTribesMap);
            if (currentTribes.length === 0) currentTribes.push({ id: 't1', name: '', color: '#ef4444' });

            setTargetTribes(currentTribes);

            const initialAssignments = {};
            contestants.filter(c => !c.is_eliminated).forEach(c => {
                  const tName = c.tribe_name || 'Merge';
                  initialAssignments[c.id] = tName;
            });

            setPlayerAssignments(initialAssignments);
            setShowTribeEditor(true);
      };
      ...
      const saveTribeUpdates = async () => {
            ...
            if (unassignedCount > 0) {
                  ...
                  if (!window.confirm("Save these tribe assignments live?")) return;
            }
      ...
      const toggleMaintenanceMode = async () => {
            ...
            await supabase.from('game_settings').update({ is_maintenance_mode: newState }).neq('id', -1);
      ...
      // NEW: Immersive Mode Condition
      const isImmersiveMode = isVotingPhase && !isAdmin;
      ...
      return (
            <div style={{ backgroundColor: '#020617', color: 'white', minHeight: '100vh', width: '100%', boxSizing: 'border-box', paddingBottom: isImmersiveMode ? '0px' : '180px' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                          {contestants.filter(c => !c.is_eliminated).map(c => (
                                                <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#1e293b', padding: '10px', borderRadius: '12px' }}>
                                                      <div style={{ fontWeight: 'bold', color: 'white', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                            {c.name}
                  ...
                  {/* conditionally hide the Header if immersive mode is active */}
                  {!isImmersiveMode && (
                  ...
                  {/* conditionally hide the main camp dashboard if immersive mode is active */}
                  {!isImmersiveMode && (
                  ...
                  <div style={{
                        ...
                        <div style={{
                                          ...
                                          <div style={{
                                                ...
                                                {isOvertime ? (
                                                      <div style={{ color: '#ef4444', fontSize: '1.2rem', fontWeight: '900', margin: '15px 0', letterSpacing: '1px', animation: 'pulse 1s infinite' }}>
                                                            ⚠️ VOTE LOCK IMMINENT - CLOSING ANY SECOND! ⚠️
                                                      </div>
                              ...
                              {/* Conditionally hide the Graveyard during Immersive Council */}
                              {!isImmersiveMode && (
